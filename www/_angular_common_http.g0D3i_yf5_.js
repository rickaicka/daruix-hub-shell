import { d as Me, g as Oe, h as De } from "@nf-internal/chunk-GMKLLVOI";
import { a as W, e as Ne, h as ae } from "@nf-internal/chunk-C4FLJMU7";
import * as h from "@angular/core";
import { \u0275RuntimeError as A, inject as v, NgZone as xt, DestroyRef as Nt, \u0275formatRuntimeError as Mr, InjectionToken as F, \u0275TracingService as Mt, runInInjectionContext as fe, PendingTasks as Ue, CSP_NONCE as Ot, DOCUMENT as je, EnvironmentInjector as Dt, makeEnvironmentProviders as At } from "@angular/core";
...
function Tt(e, r) { let i = r, { isCacheActive: t } = i, n = Ne(i, ["isCacheActive"]), { transferCache: o, method: s } = e; return !(!t || o === !1 || mr(e) || s === "POST" && !n.includePostRequests && !o || s !== "POST" && !fr.includes(s) || !n.includeRequestsWithAuthHeaders && gr(e) || vt(e.headers) || vr(e.cache) || n.filter?.(e) === !1); }
...
function yr(e, r) { let t = J(oe); if (!Tt(e, t))
    ...
    n.set(i, { [ht]: y === "arraybuffer" || y === "blob" ? Er(f) : f, [ft]: wr(p, c), [pt]: T, [yt]: b, [gt]: s, [mt]: y });
} })) : a; }
...
function vr(e) { return e === "no-cache" || e === "no-store"; }
function wr(e, r) { if (!r)
    return {}; let t = {}; for (let n of r) {
    let o = e.getAll(n);
    o !== null && (t[n] = o);
} return t; }
function ot(e) { return [...e.keys()].sort().map(r => `${r}=${e.getAll(r)}`).join("&"); }
function wt(e, r) { let { params: t, method: n, responseType: o } = e, s = ot(t), i = e.serializeBody(); i instanceof URLSearchParams ? i = ot(i) : typeof i != "string" && (i = ""); let d = [n, o, r, i, s].join("|"), a = Pr(d); return ir(a); }
function Er(e) { let r = new Uint8Array(e), t = 32768, n = ""; for (let o = 0; o < r.length; o += t) {
...
function Pr(e) { it ??= new TextEncoder; let r = it.encode(e), t = 1779033703, n = 3144134277, o = 1013904242, s = 2773480762, i = 1359893119, d = 2600822924, a = 528734635, l = 1541459225, p = r.length * 8, f = (r.length + 8 >> 6) + 1 << 6, T = new Uint8Array(f); T.set(r), T[r.length] = 128; let b = new DataView(T.buffer), R = p >>> 0, y = p / 4294967296 >>> 0; b.setUint32(f - 8, y, !1), b.setUint32(f - 4, R, !1); let c = new Uint32Array(64); for (let u = 0; u < f; u += 64) {
    ...
    for (let E = 0; E < 64; E++) {
        let j = ((w >>> 6 | w << 26) ^ (w >>> 11 | w << 21) ^ (w >>> 25 | w << 7)) >>> 0, se = (w & x ^ ~w & k) >>> 0, S = U + j + se + Rr[E] + c[E] >>> 0, ie = ((m >>> 2 | m << 30) ^ (m >>> 13 | m << 19) ^ (m >>> 22 | m << 10)) >>> 0, Rt = (m & P ^ m & g ^ P & g) >>> 0, Pt = ie + Rt >>> 0;
...
var Gr = (() => { let e = ne("json"); return e.arrayBuffer = ne("arraybuffer"), e.blob = ne("blob"), e.text = ne("text"), e; })();
...
} }; return new xe(o, () => xr(t, e), n?.defaultValue, n?.debugName, n?.parse, n?.equal, a); }; }
function xr(e, r) { let t = typeof e == "function" ? e() : e; if (t === void 0)
...
export { Z as FetchBackend, me as HTTP_INTERCEPTORS, ut as HTTP_TRANSFER_CACHE_ORIGIN_MAP, G as HttpBackend, ve as HttpClient, rr as HttpClientJsonpModule, tr as HttpClientModule, er as HttpClientXsrfModule, Q as HttpContext, de as HttpContextToken, N as HttpErrorResponse, D as HttpEventType, C as HttpFeatureKind, re as HttpHandler, V as HttpHeaderResponse, M as HttpHeaders, I as HttpParams, L as HttpRequest, _ as HttpResponse, z as HttpResponseBase, Ve as HttpStatusCode, q as HttpUrlEncodingCodec, ge as HttpXhrBackend, Re as HttpXsrfTokenExtractor, we as JsonpClientBackend, Zt as JsonpInterceptor, Gr as httpResource, et as provideHttpClient, Ht as withFetch, Qt as withInterceptors, tt as withInterceptorsFromDi, nt as withJsonpSupport, rt as withNoXsrfProtection, qt as withRequestsMadeViaParent, he as withXsrfConfiguration, Te as \u0275HTTP_ROOT_INTERCEPTOR_FNS, ee as \u0275HttpInterceptingHandler, be as \u0275REQUESTS_CONTRIBUTE_TO_STABILITY, $r as \u0275withHttpTransferCache };
